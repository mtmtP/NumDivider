...
let point = 0;
let opacity = 0;
//const soundCorrect = new Audio('sound/correct3.mp3');
//const soundIncorrect = new Audio('sound/incorrect3.mp3');
...
function tweet(){
    ...
    anchor.setAttribute(
        'data-text',
         `素数ゲーム<${level.name}>で${point}ptを獲得。${word}`
        );
...
素数の答え合わせがバグってる
    素数が出れば、選択してなくても加点されてる
    
答え合わせの方法を再考
    どれでも割れなかったら素数という考え方は合ってるのか？
    minとmaxの間で素数配列を作る？
    =>素数配列が即ち選択肢ではない

    クラスを変える表示用の答え合わせ（押したボタンに関係なく）    
        選択肢配列で順に答え合わせをすればよい
        素数選択肢のみ、別の答え合わせが必要
        ＝＞素数判別関数を作成する。
    
    押したボタンが正解かどうかを判定する関数
        if （設問が素数　&&　押したボタンのIDが0なら）{
            ピンポン鳴らす
            平方根が加点
        }else if(押したボタンのバリューで設問を割れたら){
            ピンポン鳴らす
            加点
        }else{
            ブッブ鳴らす
